...
MIN_SERVO = 0
MAX_SERVO = 180
ZERO_SERVO = int((MAX_SERVO - MIN_SERVO) / 2.)
...
class RadiansToServo:
    def __init__(self):
        self.radian_sub = rospy.Subscriber("controller", Controller, self.msg_in, queue_size=10)
        ...
        initial_msg.left_finger = ZERO_SERVO
        initial_msg.right_shoulder = ZERO_SERVO
        initial_msg.right_elbow = ZERO_SERVO
        initial_msg.right_finger = ZERO_SERVO

        self.servo_pub.publish(initial_msg)

    def cap_and_map(self, controller_axes_values):
        if (len(radian_values) < 4):
            print(f"WARNING: Too few joint values! Expected {4} got {len(controller_axes_values)}")

        capped = [max(min(MAX_CONTROL, entry), MIN_CONTROL) for entry in controller_axes_values]

        mapped = []
        for i in range(len(capped)):
            jmin_limit, jmax_limit = JOINT_SERVO_LIMITS[i]
            if (jmin_limit < MIN_SERVO): print(f"WARN: Servo {i} min value of {jmin_limit} is below global min {MIN_SERVO}")
            if (jmax_limit < MIN_SERVO): print(f"WARN: Servo {i} max value of {jmax_limit} is below global min {MAX_SERVO}")
            mapped.append(map_between_ranges(capped[i], MIN_CONTROL, MAX_CONTROL, jmin_limit, jmax_limit))
        return mapped
